...
rot_angle = linspace(0,pi/4,5).';
%rot_angle = pi/7;
dress_array = linspace(0,0.8,2);
iitr = 0;
...
for jj = 1:length(dress_array)
    for ii = 1:length(rot_angle)
       ...
       % clc;
        ...
        if jj == 1 && ii == 1
            para = a_para_init('aa',dress_array(jj),'gmr',0.1,'gmc',0,'Ny',100,'Nth',100,'FSrot_angle',rot_angle(ii),'jobdis','C4rot');
        else
             para = a_para_renew(para,'aa',dress_array(jj),'gmr',0.1,'gmc',0,'FSrot_angle',rot_angle(ii));
        end
        ...
        res1.ra = rot_angle;
        res1.dress = dress_array;
        res1.gmc = 0;
        res1.gmr = 0.1;
        res1.Vy = Phiy;
        save(['../../../TwoD_DATA_V42/',para.time,'-',para.jobdis,'/res1.mat'],"res1");
    end
end